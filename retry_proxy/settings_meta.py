...
以 .env.example 全集为准，每项描述键的类型、默认值、枚举、
分组、敏感标记、数值范围与生效类别。配置页 GET/POST /admin/settings、
校验逻辑与 tests/test_settings_meta.py 的一致性断言均以此表为唯一来源
"""
...
class ConfigItem:
    ...
    unit: str = ""
    # 数值范围（仅 int/float 项生效，闭区间）；None 表示不限制。
    # 范围应与 application.lifespan 的启动校验保持一致，避免配置页可
    # 提交启动期拒绝的值（热更新项尤其如此）。
    min: float | None = None
    max: float | None = None
...
CONFIG_ITEMS: list[ConfigItem] = [
    ...
    ConfigItem("LISTEN_PORT", "int", "8080", group="服务与访问控制",
               apply=RESTART, min=1, max=65535,
               description="监听端口（compose 端口映射需同步修改）", name="监听端口"),
    ...
    ConfigItem("SETTINGS_PAGE_ENABLED", "bool", "false", group="服务与访问控制",
               ...
               name="配置页面开关"),
    ConfigItem("API_DOCS_ENABLED", "bool", "false", group="服务与访问控制",
               apply=RESTART, description="是否公开 /docs、/redoc 和 /openapi.json；生产环境建议保持关闭",
               name="API 文档开关"),
    ConfigItem("ADMIN_COOKIE_SECURE", "bool", "false", group="服务与访问控制",
               ...
               name="Cookie 安全标记"),
    ConfigItem("ADMIN_LOGIN_MAX_ATTEMPTS", "int", "5", group="服务与访问控制",
               apply=RESTART, min=0, description="登录连续失败达到该次数后触发锁定；0=关闭限速",
               name="登录失败上限"),
    ConfigItem("ADMIN_LOGIN_LOCKOUT_SECONDS", "float", "300", group="服务与访问控制",
               apply=RESTART, unit="秒", min=0, description="登录锁定持续时间（秒）；重复锁定逐次翻倍，上限 8 倍",
               name="登录锁定时长"),
    ...
    ConfigItem("IP_AUTO_BAN_THRESHOLD", "int", "20", group="服务与访问控制",
               apply=RESTART, min=0, description="窗口内访问不同路径达到该数量时动态封禁；0=关闭",
               name="动态封禁阈值"),
    ConfigItem("IP_AUTO_BAN_WINDOW", "float", "10", group="服务与访问控制",
               apply=RESTART, unit="秒", min=0, description="动态封禁检测窗口（秒）", name="封禁检测窗口"),
    ConfigItem("IP_AUTO_BAN_DURATION", "float", "0", group="服务与访问控制",
               apply=RESTART, unit="秒", min=0, description="动态封禁持续时间（秒）；0=永久封禁",
               name="封禁持续时间"),
    ...
    ConfigItem("RETRY_INTERVAL", "float", "1.0", group="重试与退避",
               apply=HOT, unit="秒", min=0, description="非 429 错误的重试间隔（秒）；退避时作为基数", name="重试间隔"),
    ...
    ConfigItem("RETRY_BACKOFF_MAX", "float", "60", group="重试与退避",
               apply=HOT, unit="秒", min=0, description="非 429 指数退避最高等待（秒）", name="退避上限"),
    ConfigItem("RETRY_INTERVAL_429", "float", "5.0", group="重试与退避",
               apply=HOT, unit="秒", min=0, description="429 专用重试间隔（秒）；优先尊重 Retry-After",
               name="429 重试间隔"),
    ...
    ConfigItem("RETRY_BACKOFF_MAX_429", "float", "60", group="重试与退避",
               apply=HOT, unit="秒", min=0, description="429 指数退避最高等待（秒）", name="429 退避上限"),
    ConfigItem("RETRY_AFTER_MAX", "float", "0", group="重试与退避",
               apply=HOT, unit="秒", min=0, description="上游 Retry-After 头封顶值（秒）；0 表示不封顶，完全尊重",
               name="Retry-After 封顶"),
    ...
    ConfigItem("MAX_CONCURRENT", "int", "10", group="竞速模式",
               apply=HOT, min=1, description="竞速模式单轮最大并发数", name="最大并发数"),
    ...
    ConfigItem("KEY_POOLS", "str", "", group="号池来源与鉴权", secret=True, attr="key_pools_raw",
               ...
               name="号池 Key 列表"),
    ConfigItem("KEY_POOL_ALLOW_PRIVATE_BASE_URL", "bool", "false", group="号池来源与鉴权",
               apply=RESTART, description="允许号池连接/手动添加使用私有或内网上游地址；默认禁止以防范 SSRF",
               name="允许内网上游"),

    # ---------------------------------------------------------------- 号池熔断与选择
    ConfigItem("KEY_COOLDOWN", "float", "30", group="号池熔断与选择",
               apply=HOT, unit="秒", min=0, description="旧版兼容值；未配置 KEY_COOLDOWN_5XX 时作为默认冷却时间",
               name="默认冷却时间"),
    ConfigItem("KEY_COOLDOWN_5XX", "float", "30", group="号池熔断与选择",
               apply=HOT, unit="秒", min=0, description="5xx 错误冷却时间（秒）", name="5xx 冷却时间"),
    ConfigItem("KEY_COOLDOWN_429", "float", "60", group="号池熔断与选择",
               apply=HOT, unit="秒", min=0, description="429 错误冷却时间（秒）", name="429 冷却时间"),
    ConfigItem("KEY_COOLDOWN_AUTH", "float", "1800", group="号池熔断与选择",
               apply=HOT, unit="秒", min=0, description="鉴权错误冷却时间（秒）", name="鉴权错误冷却"),
    ConfigItem("KEY_COOLDOWN_MAX", "float", "3600", group="号池熔断与选择",
               apply=HOT, unit="秒", min=0, description="冷却时间上限（秒）", name="冷却上限"),
    ...
    ConfigItem("KEY_STICKY", "float", "120", group="号池熔断与选择",
               apply=HOT, unit="秒", min=0, description="key 粘性时间（秒）；0=禁用", name="Key 粘性时间"),
    ConfigItem("KEY_POOL_WAIT_TIMEOUT", "float", "120", group="号池熔断与选择",
               apply=HOT, unit="秒", min=0, description="所有候选 key 均熔断时的最长等待时间（秒）；超时返回 503，0=不限制",
               name="熔断等待超时"),
    ConfigItem("KEY_TTFT_STALE_AFTER", "float", "300", group="号池熔断与选择",
               apply=HOT, unit="秒", min=0, description="真实请求样本有效期（秒）", name="样本有效期"),
    ConfigItem("KEY_TTFT_RETEST_INTERVAL", "float", "60", group="号池熔断与选择",
               apply=HOT, unit="秒", min=0, description="便宜分组轮转复测间隔（秒）", name="复测间隔"),
    ConfigItem("KEY_TTFT_CONFIRMATIONS", "int", "2", group="号池熔断与选择",
               apply=HOT, min=1, description="慢分组升级所需连续样本数", name="升级确认样本数"),
    ...
    ConfigItem("KEY_CACHE_MISS_COOLDOWN", "float", "3600", group="号池熔断与选择",
               apply=HOT, unit="秒", min=0, description="缓存熔断冷却时间（秒）；0=关闭", name="缓存熔断冷却"),
    ...
    ConfigItem("KEY_POOL_SYNC_INTERVAL", "int", "300", group="在线同步",
               apply=RESTART, unit="秒", min=0, description="自动同步周期（秒）；0=仅手动同步", name="自动同步周期"),
    ConfigItem("KEY_POOL_CREATE_DELAY", "float", "1.5", group="在线同步",
               apply=RESTART, unit="秒", min=0, description="同步创建 key 的间隔（秒）", name="建号间隔"),
    ...
    ConfigItem("LOG_RETENTION_DAYS", "int", "30", group="日志",
               apply=RESTART, unit="天", min=0, description="日志保留天数；0=不清理", name="日志保留天数"),
    ...
    ConfigItem("DLP_MAX_BODY_BYTES", "int", "16777216", group="请求正文敏感信息防护",
               apply=HOT, min=1, description="最大扫描字节数；redact/block 模式下超限返回 413",
               name="最大扫描字节"),
    ConfigItem("DLP_DECODE_DEPTH", "int", "2", group="请求正文敏感信息防护",
               apply=HOT, min=0, max=8, description="Base64/hex/percent 编码递归扫描层数；0 层表示关闭解码",
               name="解码扫描层数"),
    ConfigItem("DLP_DECODE_MAX_CANDIDATES", "int", "100", group="请求正文敏感信息防护",
               apply=HOT, min=1, description="编码解码候选上限", name="解码候选上限"),
    ConfigItem("DLP_DECODE_MAX_BYTES", "int", "1048576", group="请求正文敏感信息防护",
               apply=HOT, min=1, description="编码解码扫描字节上限", name="解码扫描字节上限"),
    ConfigItem("DLP_KNOWN_SECRET_MIN_LENGTH", "int", "8", group="请求正文敏感信息防护",
               apply=HOT, min=1, description="号池 Key 小于该长度时不做精确内容匹配，避免短值误报",
               name="精确匹配最小长度"),
