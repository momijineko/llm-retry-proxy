...
首 Token 指标在同一在线分组的 Key 间共享，保存在运行时内存中；同步热更新会保留未删除 Key 的样本，进程重启后重新学习。“首 Token 优先”会按倍率探测所有无样本分组，并在运行期间低频复测其它分组；“兼顾两者”则限制为达到目标所需的逐级探测和更低倍率复测，避免用真实业务请求主动探测不必要的高倍率分组。静态 CSV 号池继续使用最低倍率优先，不受在线来源策略影响。
