...
- **兼顾两者**：在实测首 Token 不超过“可接受首 Token 上限”的分组中选择倍率最低者；若全部超过上限，则选择实测最快者。

首 Token 指标在同一在线分组的 Key 间共享，保存在运行时内存中；同步热更新会保留未删除 Key 的样本，进程重启后重新学习。无样本的分组会先按倍率探测，运行期间也会低频复测其它分组，避免旧样本长期固化选择。静态 CSV 号池继续使用最低倍率优先，不受在线来源策略影响。

号池页面的“检测可用性”只在手动点击时执行，不会后台轮询模型。检测最多同时发起 2 个请求，按分组依次尝试 Key；组内任一 Key 成功后立即停止检测该组。检测期间页面会读取本地管理状态，已成功分组的首 Token 无需等待整批结束即可显示；状态刷新不会向上游增加请求。模型检测使用页面指定的检测模型调用上游 `POST /v1/chat/completions`，只包含固定的最小提示并设置 `max_tokens=1`。真实生成返回 `2xx/3xx` 才视为可用，同时将不含并发排队时间的完成耗时写入该分组的首 Token 估算；HTTP 错误、超时或网络异常均视为失败。只有组内全部 Key 失败时才熔断整个分组。检测熔断沿用 `KEY_COOLDOWN_5XX`，到期自动恢复，也可以从页面解除单个分组或全部分组的熔断。
