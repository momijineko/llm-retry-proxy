...
6. 429 带 `Retry-After` 时取 `max(指数熔断时间, Retry-After)`，即使超过 `KEY_COOLDOWN_MAX` 也尊重上游时间；可通过 `RETRY_AFTER_MAX` 为 `Retry-After` 设置秒数上限（默认 `0`=不封顶），超过上限时按上限估算，基础熔断时间仍会生效
7. **粘性保持**（`KEY_STICKY` 默认 120 秒）：只有收到成功响应后才建立当前 key 的粘性会话。静态号池和其它策略继续按成功请求续期；“兼顾三者”使用固定窗口，不因普通成功请求续期，并且窗口内仍按复测间隔借一个真实请求检查单个低倍率分组。失败候选不会建立窗口，当前 key 被熔断时仍立即切换。设为 `0` 禁用
...
的上游。填写上游地址和可选代理前缀后，可以每行输入一个 Key；每行支持
`key|标签|倍率` 格式，也可以为整批 Key 设置共同分组、倍率和模型规则。
同一批次及已有号池中的重复 Key 会被跳过。
代理前缀仅在上游地址已经是默认上游或已有环境路由时可以留空；其它上游
必须填写独立前缀，避免创建后没有任何请求路径能够命中。

手动号池会立即进入运行时路由，不参与后台同步。它不能与同一上游地址的
在线同步来源并存；代理前缀冲突时添加操作会失败，不会留下不可达的号池。
向已有手动号池追加 Key 时沿用原 `provider` 和代理前缀；如需更换，应先删除
该手动来源再重新创建。
删除最后一个手动 Key 会同时删除该手动来源并恢复同地址的静态号池。手动
Key 与在线同步得到的 Key 使用同一状态文件及加密策略：配置
`KEY_POOL_SYNC_SECRET`（或回退使用 `ADMIN_PASSWORD`）后密文落盘，两者均未
配置时为兼容旧部署而明文落盘。
...
分组目录支持多选创建 Key、一键补齐缺失分组，以及选择分组清空远程 Key（操作前会二次确认）。每次正常同步都会识别已经删除或停用分组中的遗留 Key，并自动调用上游删除；删除失败时会记录警告并在后续同步重试。管理页只展示同步后仍然有效的分组，不再展示失效分组。新建 Key 默认直接使用分组名；适配器仍可通过显式 `name_prefix` 选项覆盖。Key 列表支持按分组或倍率排序，排序只影响展示。列表中的单条 Key 可以手动停用或重新启用；停用后它仍会显示和参与在线同步，但不会进入运行时号池，因此后续请求不会自动路由到该 Key。人工停用状态保存在同步状态文件中，重启和再次同步后继续生效。

每个在线同步来源可以在号池页面选择运行时分组调度策略：

- **最低倍率优先**：保持原有行为，按同步得到的倍率从低到高选择。
- **首 Token 优先**：根据成功响应的首个有效响应体或 SSE Token 计算分组级 EWMA，优先选择最快的分组。配置外部数据源后，可用“外部参考强度”将外部 TTFT 作为少量先验样本；本地真实样本越多，外部影响越小。
- **缓存命中优先**：根据成功 Responses 响应明确返回的缓存 Token 和输入 Token，按“端点类型 + 模型 + 分组”实时累计 CH（`cached_tokens / input_tokens`）。可为每个来源设置可接受 CH 下限；当前分组只有连续达到确认次数的长输入响应低于下限后才会切换，中间任何一次达标都会清零计数。确认切换后优先选择达到下限且 CH 最高的分组，没有达标分组时再采样未知组或选择当前最优组。命中率相同时优先选择倍率更低的分组，未返回缓存统计的响应不会进入计算。
- **兼顾三者**：倍率决定优先使用的成本层级，TTFT 滞回控制层级间的升级和恢复；同一倍率有多个可用分组时，缓存下限也使用连续确认，确认前保持当前分组，确认后选择本倍率内达到下限且 CH 最高的分组。CH 不会单独把请求推到更高倍率。当前成本层连续出现足够次数的慢样本后才升级到下一倍率；升级后，系统按时间借用单个真实请求轮转复测更低倍率分组。低倍率探测的真实首 Token 低于恢复线时立即降回。目标上限默认使用 10% 滞回：例如目标 5 秒时，连续超过 5.5 秒才升级，单次低倍率探测低于 4.5 秒即可降回，位于中间区间时保持当前成本层。配置外部数据源后，“外部复测权重”会混合倍率排名和外部 TTFT 排名来安排先测哪个低倍率组；`0%` 完全按倍率轮转，`100%` 完全按外部 TTFT 排序，默认 `50%`。
...
代理还会读取成功 Responses 响应中上游明确返回的 `usage.input_tokens_details.cached_tokens`。同一 Codex 会话（按 `thread_id`、`session_id` 或 `prompt_cache_key` 识别）在同一“端点类型 + 模型 + 分组”连续 3 次输入不少于 1024 token 时仍为零，且当前存在其它可用分组，代理会把该分组整体熔断 1 小时；命中缓存会清零该分组的连续计数并立即解除仅由缓存未命中触发的熔断。短输入、缺少缓存统计或会话标识、请求失败、不同新会话的首次请求，以及只有一个可用分组时都不会触发。阈值、输入下限和熔断时间可分别通过 `KEY_CACHE_MISS_THRESHOLD`、`KEY_CACHE_MISS_MIN_INPUT_TOKENS`、`KEY_CACHE_MISS_COOLDOWN` 调整；阈值或熔断时间设为 `0` 可关闭。

真实首 Token 指标按“端点类型 + 模型 + 在线分组”隔离，并在同一分组的 Key 间共享，保存在运行时内存中；同步热更新会保留仍存在的请求视图，进程重启后重新学习。样本默认 5 分钟后过期；“首 Token 优先”直接选择当前真实或融合 TTFT 最低的分组，不使用成本档位升降和外部复测权重；“兼顾三者”始终保留最近成功的成本层作为可靠锚点，在层内按带连续确认的 CH 下限选择分组，并按倍率从低到高轮转复测锚点以下的分组。固定粘性窗口不会阻止定时复测，窗口到期也不会清空锚点或从最低倍率连续重扫。一次低倍率探测失败后立即回到锚点或更高倍率，本次请求不会继续扫描其它低倍率组；下个复测周期从游标的下一分组继续。低倍率探测成功但未达到恢复线时仍保留原锚点，达到恢复线则立即切换并开始新的固定窗口。同一时刻只会借一个真实请求进行复测，不会在后台额外生成请求。静态 CSV 号池继续使用最低倍率优先，不受在线来源策略影响。

号池页面会按活跃的“端点类型 + 模型”显示当前主分组；TTFT 策略显示真实 TTFT、样本状态和低倍率组复测进度，缓存命中策略显示当前 CH、累计 Token、样本数和全部候选分组的实时 CH。页面每 5 秒读取一次内存状态，刷新不会向上游增加请求。下方 Key 表按分组汇总全部活跃请求类型的 CH；“延时观测”同时列出真实请求 TTFT 汇总和手动检测的完整响应耗时，两者独立统计。修改可接受上限时，页面会即时显示实际升级线、降回线和连续确认次数。
...
“首 Token 优先”将外部 TTFT 作为可衰减的先验样本。页面提供“关闭 / 弱 / 中 / 强”四档外部参考强度，分别相当于 `0 / 1 / 2 / 5` 个先验样本，默认“中”。用于排序的 TTFT 为 `(本地 EWMA × 本地样本数 + 外部 TTFT × 先验样本数) / (本地样本数 + 先验样本数)`；没有本地样本时使用外部值进行冷启动，本地样本增加后外部占比自动下降。外部接口声明的样本数不会直接成为权重，因为不同数据源的采样口径不可比。
