...
7. **粘性保持**（`KEY_STICKY` 默认 120 秒）：只有收到成功响应后才建立当前 key 的粘性会话，后续请求会不断续期；窗口内保持当前 key 和分组，不执行倍率、首 Token 或低倍率恢复复测。失败候选不会建立或续期窗口；只有持续空闲超过 2 分钟，或当前 key 被熔断，下一次才重新执行分组选择。设为 `0` 禁用
...
- **兼顾两者**：优先使用最低倍率分组。当前分组连续出现足够次数的慢样本后才升级到下一倍率；升级后，系统按时间借用单个真实请求复测低倍率分组，连续恢复后再降回。目标上限默认使用 10% 滞回：例如目标 5 秒时，连续超过 5.5 秒才升级，连续低于 4.5 秒才降回，位于中间区间时保持当前分组。

真实首 Token 指标按“端点类型 + 模型 + 在线分组”隔离，并在同一分组的 Key 间共享，保存在运行时内存中；同步热更新会保留仍存在的请求视图，进程重启后重新学习。样本默认 5 分钟后过期；“首 Token 优先”使用真实请求更新过期或未知分组，“兼顾两者”只复测当前选择以下的更低倍率分组。粘性窗口有效时保持当前分组并跳过复测，窗口到期后才恢复调度。一次低倍率恢复探测失败后立即回到原倍率或更高倍率，不会在同一请求中继续扫描其它低倍率组。同一时刻只会借一个真实请求进行复测，不会在后台额外生成请求。静态 CSV 号池继续使用最低倍率优先，不受在线来源策略影响。
