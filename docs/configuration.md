...
| `RETRY_AFTER_MAX` | `0` | 上游 `Retry-After` 头封顶值（秒）；`0` 表示不封顶，完全尊重上游。非 0 时，Retry-After 对重试等待与 key 熔断的贡献均不超过该值；最终时长仍可能受基础间隔、指数退避和基础熔断配置影响 |
...
| `KEY_STICKY` | `120` | 成功 key 的分组粘性时间（秒）；“兼顾三者”使用不续期的固定窗口且窗口内仍允许单分组定时复测，其它策略沿用空闲续期；失败候选不建立窗口，当前 key 熔断时仍立即切换；`0` = 禁用 |
...
| `KEY_TTFT_RETEST_INTERVAL` | `60` | “兼顾三者”轮转复测单个低倍率分组的最小间隔（秒） |
...
| `KEY_CACHE_HIT_CONFIRMATIONS` | `3` | 缓存命中下限策略连续多少次低于下限后才切换分组；中间命中达标会清零；最小为 `1` |
