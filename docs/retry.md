...
- **429 特殊处理**：429 使用 `RETRY_INTERVAL_429`（默认 5s）而非 `RETRY_INTERVAL`；若上游 429 响应携带 `Retry-After` 头（秒数或 HTTP 日期），则优先按该头等待。可通过 `RETRY_AFTER_MAX`（默认 `0`=不封顶）为其设置秒数上限，避免异常巨大的 `Retry-After` 导致长时间熔断或等待；封顶同时作用于重试等待与 key 熔断。
